import { Component, ChangeDetectorRef, ElementRef, QueryList, ViewChild, ViewChildren } from '@angular/core';
...
trailerCoverUrl: string = '';
   ...
   */
ngOnInit() {
  this.isMuted = sessionStorage.getItem('muted') === 'true';
  ...
  this.http.get<any[]>(this.urls.baseUrl + '/movie/trailer/')
    .subscribe(data => {
      if (data.length > 0) {
        ...
        this.trailerCoverUrl = data[0]['cover'] || '';
        if (this.videoRef) {
